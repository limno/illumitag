...
data = read.table('/My Folders/PHD/Server/Uppmax_home/ILLUMITAG/views/projects/test/otus/denovo/table/table_filtered.csv', header=TRUE, sep='\t', row.names='OTUID')
otu_table = read.table('/My Folders/PHD/Server/Uppmax_home/ILLUMITAG/views/clusters/domenico/otus/uparse/otu_table.csv', header=TRUE, sep='\t', row.names='X')
otu_table = read.table('views/clusters/domenico/otus/uparse/otu_table.csv', header=TRUE, sep='\t', row.names='X')

library(vegan)
ord = metaMDS(otu_table, distance='horn', trymax=200)